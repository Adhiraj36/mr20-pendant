// The React Native ThinkingOrb — vendored from
// github.com/Jakubantalik/thinking-orbs (MIT, ports/react-native), with one
// local addition: `renderSize` scales the recorded picture as vectors, so a
// hero-sized orb stays crisp while keeping the 64 dp preset's tuning.
//
...
export function ThinkingOrb({
  ...
  accessibilityLabel,
  style
}: ThinkingOrbProps & { renderSize?: number }) {
  const drawn = renderSize ?? size;
  const scale = drawn / size;
  ...
  }, [mode, opts, size, scale, drawn, dark, effSpeed, paused, reduced, appActive, paints, rgba]);
  ...
  return (
    <View
      ...
      style={[{ width: drawn, height: drawn }, style]}
    >
